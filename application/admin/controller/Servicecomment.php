...
class Servicecomment extends Base {
    ...
    public function detail(){
        $id = I('get.id/d');
        $res = M('ServiceComment')->where(array('comment_id'=>$id))->find();
        if(!$res){
    ...
    public function del(){
        $id = I('get.id/d');
        $row = M('ServiceComment')->where(array('comment_id'=>$id))->delete();
        if($row){
            $this->success('删除成功');
        }else{
            $this->error('删除失败');
...
//            exit(DB::getLastSql());
    ...
    public function ajaxindex(){
        ...
        {
            $goods_id_arr = get_arr_column($comment_list, 'service_id');
            $goods_list = M('Service')->where("id", "in" , implode(',', $goods_id_arr))->getField("id,name");
        }
